...
<head>
    <title>Message - </title>
    <?php header(); ?>  <!-- header présent sur toutes les pages (connexion avec bootstrap) -->
</head>
                                ...
                                <div class="col-lg-3 col-md-4 chat-list-wrapper px-0">
                                    <div class="chat-list-item-wrapper">
                                        ...
                                        foreach($listConv as $conv){
                                            $req3 = $pdo->prepare('SELECT * FROM message where idm = :idm order by time desc limit 1');
                                            $req3->bindValue(':idm', $conv["idm"], PDO::PARAM_INT);
                                            $req3->execute();
                                            ...
                                            if($result3["idreceveur"] == $uid){
                                                $idCorrespondantConv = $result3["idenvoyeur"];
                                            }else{
                                                $idCorrespondantConv = $result3["idreceveur"];
                                            }
                                            ...
                                            $req6 = $pdo->prepare('SELECT * FROM user WHERE idu = :idu');
                                            $req6->bindValue(':idu', $idCorrespondantConv, PDO::PARAM_INT);
                                            $req6->execute();
                                            $infoCorresConv = $req6->fetch();
                                            // on recupere les informations de l'user 

                                            ?>
                                            <a  style="outline: none; text-decoration: none;"  href="message.php?idc=<?=$conv["idm"]?>"> <!--  cliquer sur la conevrsation? -->
                                            <div class="list-item">
                                                <div class="profile-image">
                                                    <img class="img-sm rounded-circle" src="../<?php if($infoCorresConv["photo"]){  echo $infoCorresConv["avatar"]; }else{ echo "image/avatarbasique.png";} ?>" alt="">
                                                </div>
                                ...
                                <div class="col-lg-6 col-md-8 px-0 d-flex flex-column">
                                        ...
                                        <?php   }else{ ?>
                                                    <div class="chat-bubble outgoing-chat" >
                                                        ...
                                                        <div class="sender-details">
                                                            <img class="sender-avatar img-xs rounded-circle" src="../<?php if($infoUser["photo"]){  echo $infoUser["photo"]; }else{ echo "image/avatarbasique.png";} ?>" alt="profile image">
                                                            <p class="seen-text">Message envoyé : <?php
                                                                $timeMess = date('U')-$mess["time"];
                                                                if($timeMess < 3600){
                                                                    echo "il y a ".date("i", $timeMess)." minutes";
